# Implementation and folder structure was inspired by code from https://github.com/DougTrajano/drl-flappy-bird
# Though we wrote most of the code ourselves
import numpy as np
...
def save_training_results(agent, trainer, run_dir, models_dir, seed, final=False):
    ...
    final_model_path = os.path.join(models_dir, "DQN_final.pt")
...
def create_checkpoint_callback(agent, run_dir, checkpoints_dir, seed, checkpoint_frequency=10):
    ...
    def checkpoint_callback(trainer, episode):
        ...
        try:
            checkpoint_path = os.path.join(checkpoints_dir, f"DQN_checkpoint_ep{episode}.pt")
...
def main():
    ...
    try:
        trainer.run(
            ...
            output_path=os.path.join(models_dir, "DQN_best.pt"),
            run_dir=run_dir,
            agent_name=run_name
        )
