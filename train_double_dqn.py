...
import csv
from src.metrics import MetricsTracker  # Import MetricsTracker
...
def create_run_directory(feature_extractor, suffix=""):
    ...
    run_name = f"DoubleDQN_{feature_extractor}{suffix}_{timestamp}"
...
def plot_scores(scores, window_size=100, feature_extractor='resnet', suffix='', save_path=None):
    ...
    plt.title(f'Double DQN Training Progress ({feature_extractor.capitalize()})')
    plt.xlabel('Episode')
    plt.ylabel('Score')
    plt.legend()
    ...
    if save_path:
        ...
        plt.savefig(f'training_progress_double_dqn_{feature_extractor}{suffix}.png')
...
def save_training_results(agent, trainer, run_dir, models_dir, plots_dir, feature_extractor, suffix, seed, final=False):
    ...
    try:
        # Save the model
        if final:
            final_model_path = os.path.join(models_dir, f"DoubleDQN_{feature_extractor}{suffix}_final.pt")
            agent.save_model(final_model_path)
            print(f"Final model saved to: {final_model_path}", flush=True)
        
        # Plot and save the learning curve
        plot_path = os.path.join(run_dir, f"training_progress_double_dqn_{feature_extractor}{suffix}_final.png")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train Double DQN with various feature extractors on Flappy Bird')
    ...
    # We'll determine this based on the preprocessing parameters
    if args.frame_stack:
        if args.feature_extractor in ['dinov2', 'clip', 'convnext']:
            # RGB input with frame stacking
            state_shape = (3 * args.frame_stack_size, target_size[0], target_size[1])
        else:
    ...
    # Model paths
    model_path = os.path.join(models_dir, f"DoubleDQN_{args.feature_extractor}{suffix}.pt")
    best_model_path = os.path.join(models_dir, f"DoubleDQN_{args.feature_extractor}{suffix}_best.pt")
